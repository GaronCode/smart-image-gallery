 ...
 * - tap
 */
export class SimpleSwipeController {
    start = { x: 0, y: 0 };
    eventCallbacks = {
        swipeLeft: [],
        swipeRight: [],
    ...
    setEventGroup(eventName, groupEditorCallback) {
        ...
        if (EventGroup === undefined) {
            console.log(
                `Не существует события с названием ${eventName}. Список имеющихся событий: ${this.eventsName()}`
            );
            return false;
        }
    ...
    handler(x, y, event) {
        let events = [];
        if (x < this.start.x - this.offset) events.push("swipeLeft");
        else if (x > this.start.x + this.offset) events.push("swipeRight");

        if (y < this.start.y - this.offset) events.push("swipeUp");
        else if (y > this.start.y + this.offset) events.push("swipeDown");
